...
void main() {
  group('Drag and Drop Knob Consistency Tests', () {
    testWidgets(
      'When enableDragAndDrop is false, drag should be completely disabled',
      (WidgetTester tester) async {
        ...
        expect(
          ...
          false,
          reason:
              'onReorder should not be called when enableDragAndDrop is false',
        );
        expect(
          onDragStartCalled,
          false,
          reason:
              'onDragStart should not be called when enableDragAndDrop is false',
        );
        expect(
          onDragEndCalled,
          false,
          reason:
              'onDragEnd should not be called when enableDragAndDrop is false',
        );
      },
    );
    ...
    testWidgets('When enableDragAndDrop is true, all drag callbacks should work', (
      ...
      expect(
        onDragStartCalled,
        true,
        reason: 'onDragStart should be called when enableDragAndDrop is true',
      );
      expect(
        onDragEndCalled,
        true,
        reason: 'onDragEnd should be called when enableDragAndDrop is true',
      );
      // Note: onReorder might not be called if the drop position doesn't result in an actual reorder
    });

    testWidgets(
      'Demonstrates the original bug (story callbacks without enableDragAndDrop)',
      (WidgetTester tester) async {
        ...
        Widget buggyImplementation = MaterialApp(
          home: Scaffold(
            body: ReorderableTreeListView(
              ...
              onDragStart:
                  null, // Explicitly null to test the old buggy behavior
